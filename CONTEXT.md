...
*Last updated: 2026-07-06 by Claude Code (AI agent) — **Project report reformatted to match the reference + light-mode screenshots**. Docs-only change (no service code touched).*
*• Recaptured 10 student-facing screens on-device in **light mode** (device forced via `adb shell cmd uimode night no`; app follows `ThemeMode.system`): login, dashboard, complaint list, new-complaint form, AI grammar suggestion, grammar-applied + AI category (Plumbing 100%), complaint detail, updated feed after submit, statistics, profile. Saved as `docs/screenshots/ios-clean-redesign/30..39-*-light.png` (originals kept). Backend + AI service + `adb reverse` tunnel run via `scripts/start-dev.ps1 -Phone` with seeded sample data.*
*• `docs/report_assets/build_report.py`: swapped all figures to the new light screenshots (no dark refs remain), reworked Section 5 into 5 side-by-side pairs, and renumbered the in-text figure references. Matched the reference formatting: **black headings** (H1 15pt / H2 13pt, was navy), cover title 18pt, and a **two-section layout** — front matter with no header/footer, body section with a running header `Smart Complaint Management System (SCMS)  <Month Year>` and footer `Department of MCA, RVCE  Page <PAGE field>`.*
*• Regenerated `docs/SCMS_Project_Report.docx`; verified 2 sections, black heading styles, header/footer + live PAGE field, and 16 embedded images. TOC still needs a manual “Update Field” in Word to populate page numbers.*
*• KNOWN ISSUE surfaced during capture: the Flutter **submit-complaint** flow returns “Failed to submit complaint” even when the identical payload succeeds via `POST /api/complaints` (created SCMS-2026-00007 directly) — a client-side bug in the submit path, unrelated to this report task; not yet fixed.*
